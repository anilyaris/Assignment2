...
void addIf(std::vector<int>* v, int n) {
...
void MeshWithConnectivity::LoopSubdivision() {
		...
		for (size_t i = 0; i < indices.size(); ++i) {
			for (int j = 0; j < 3; ++j) {
				...
				if (new_vertices.find(edge) == new_vertices.end()) {
					...
					int neighborTri = neighborTris[i][j];
					int neighborEdge = neighborEdges[i][j];
					int v3 = neighborTri >= 0 ? indices[neighborTri][(neighborEdge + 2) % 3] : v1;
					if (neighborTri < 0) v2 = v0;
					Vec3f pos, col, norm;
	...
	std::vector<std::vector<int>> adjacencies(positions.size());

	for (int i = 0; i < indices.size(); ++i) {

		addIf(&adjacencies[indices[i].x], indices[i].y);
		addIf(&adjacencies[indices[i].x], indices[i].z);
		addIf(&adjacencies[indices[i].y], indices[i].x);
		addIf(&adjacencies[indices[i].y], indices[i].z);
		addIf(&adjacencies[indices[i].z], indices[i].x);
		addIf(&adjacencies[indices[i].z], indices[i].y);
	...
	for (int i = 0; i < adjacencies.size(); i++) {
		int n = adjacencies[i].size();
		float b = n > 3 ? 0.375f / n : (n == 3 ? 0.1875f : 0.125f);

		pos = (1.0f - n * b) * positions[i];
		col = (1.0f - n * b) * colors[i];
		norm = (1.0f - n * b) * normals[i];

		for (int j = 0; j < n; j++) {
			pos += b * positions[adjacencies[i][j]];
			col += b * colors[adjacencies[i][j]];
			norm += b * normals[adjacencies[i][j]];
		}
	...
	for (size_t i = 0; i < indices.size(); ++i) {
		...
		new_indices.push_back(Vec3i(even.x, odd.x, odd.z));
		new_indices.push_back(Vec3i(odd.x, even.y, odd.y));
		new_indices.push_back(Vec3i(odd.y, even.z, odd.z));
